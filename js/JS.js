        ...
        $("input,textarea").jqBootstrapValidation({
            ...
            submitSuccess: function($form, event) {
              ...
              //  var stuemail = $("input#email").val();
                ...
                if(!(ide==null&&stuname==null&&stugend==null&&stuadress==null&&studib==null&&stucontact==null)){
 ...
 $(document).on("click", "#busTrans", function (e) {
    //$(".selected li a").click(function () {
        $("#portfolioModal5").modal("show");
        showBusinessTransdata();
...
function showBusinessTransdata(){
        jQuery('#businessTransaction').html('');


        $.ajax({
            'url': 'php/department.php?type="level5"',
            success: function (h) {
                var txt = "<h2>Current Allocation</h2><hr class=\"star-primary\"><div  style=\"width: 100%; float:left; border:1px  ;  padding: 1em;\"> <table   class=\"table-hover table-condensed table-responsive\" style=\"width: 100%; table-layout:fixed;border-spacing: 5px;\"><thead><tr><th><center>Course Name</center></th><th><center>Number of Students Registered<center></th></tr></thead><tbody>";
                bus_info = jQuery.parseJSON(h);
                for (var i = 0; i < bus_info.length; i++) {
                    txt += " <tr  align=\"left\"><td ><button type=\"button\" style=\"word-wrap: break-word;white-space: normal;\" class=\"btn btn-primary btn-group btn-group-justified \"  >" + bus_info[i].course_name + "</button></td><td ><button type=\"button\" style=\"word-wrap: break-word;white-space: normal;\" class=\"btn btn-primary btn-group btn-group-justified \"  >" + bus_info[i].Count_Student + "</button></td></tr>"
                ...
                if (txt != "") {
                    ...
                    txt += "</tbody></table></div>";
                    jQuery('#businessTransaction').html('');
                    $("#businessTransaction").append(txt);
                }
